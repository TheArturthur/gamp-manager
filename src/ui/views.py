...
from src.prometheus import (
    ansible_add_host_to_inventory,
    add_new_project_to_prometheus,
    ansible_install_exporter,
)
from src.ui import db
from src.ui.functions import (
    check_form_data_validity,
    get_exporter_latest_version,
    create_new_project,
    template_values,
)
from src.ui.models import Projects, Exporters, Targets
...
def start():
    """
    Home page of application. User can check whether a project exists in the database.
    """
    myself = "home.html"
    if request.method == "POST":
        ...
        if project is None:
            flash(
                f'Project "{project_name}" was not found in the database! Try adding it.',
                category="error",
            )
        ...
        if "target_name" in validity_results[False]:
            ...
            # ip
            target = Targets.query.filter_by(
                project=project_name, IP=target_name
            ).first()
            return render_template(
                myself, project=project, targets=target, missing=target is None
            )
        else:
            # hostname
            target = Targets.query.filter_by(
...
def new_project():
    if request.method == "POST":
        ...
        if len(validity_results[False]) == 0:
            ...
            flash(
                f"Project {project_name} created in Datacenter {project_dc}",
                category="success",
            )
...
def get_exporters():
    if request.method == "POST":
        ...
        if len(validity_results[False]) == 0:
            ...
            new_exporter = Exporters(
                Name=exporter_name, URL=exporter_url, Latest_version=latest_version
            )
...
def targets():
    ...
    if request.method == "POST":
        ...
        print(
            f"Selection: {selected_project_name}, New Name: {new_project_name}, New DC: {new_project_datacenter}"
        )
        ...
        if selected_project_name == "new_project":
            if len(validated_results[False]) == 0:
                ...
                flash(
                    f"Project {new_project_name} created in Datacenter {new_project_datacenter}",
                    category="success",
                )
            else:
            ...
            db.session.commit()
            flash(
                f"Target {new_target_name} added to project {selected_project_name}",
                category="success",
            )
            ansible_add_host_to_inventory(target=new_target)
            add_new_project_to_prometheus(
                target=new_target,
            ...
            ansible_install_exporter(
                target=new_target, exporter_version=exporter.Latest_version
            )
        ...
        print(target_list)
        return render_template(
            "targets.html", project_list=project_list, target_list=target_list
        )
